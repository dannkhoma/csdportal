...
public class WrappedEntityComparator implements Comparator<WrappedEntity> {
    ...
    public int compare(WrappedEntity o1, WrappedEntity o2) {
        if (isPrimaryFacility(o1) && !isPrimaryFacility(o2)) {
            return -1;
        } else if (!isPrimaryFacility(o1) && isPrimaryFacility(o2)) {
            return 1;
        } else if (isDoctor(o1) && !isDoctor(o2)) {
            return -1;
        } else if (!isDoctor(o1) && isDoctor(o2)) {
            return 1;
        } else if (!isFromMinistryHR(o1) && isFromMinistryHR(o2)) {
            return -1;
        } else if (isFromMinistryHR(o1) && !isFromMinistryHR(o2)) {
            return 1;
        } else {
    ...
    public boolean isFromMinistryHR(WrappedEntity wrappedEntity) {
        return wrappedEntity.getEntityId().contains("urn:mohcc_hr_");
    }
